...
/*Value makekeypair(const Array& params, bool fHelp)
{
    ...
    do
    {
        ...
        nCount++;
    } while (nCount < 10000 && strPrefix != HexStr(key.GetPubKey().GetID()));

    if (strPrefix != HexStr(key.GetPubKey().GetID()))
        return Value::null;

    Object result;
    result.push_back(Pair("PublicKey", HexStr(key.GetPubKey().GetID())));
    //result.push_back(Pair("PrivateKey", key.GetPrivKey() ));
    return result;
}*/
